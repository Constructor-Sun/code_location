...
from preprocess.build_graph import build_graph, VERSION
from utils.benchmark.setup_repo import setup_repo
...
def get_prefix(instance_id):
    return instance_id.split('.')[1]  # 获取前缀，例如 "1fd52536"
...
if __name__ == '__main__':
    ...
    if args.train:
        if args.download_repo:
            ...
            if args.instance_id_path and osp.exists(args.instance_id_path):
                with open(args.instance_id_path, 'r') as f:
                    repo_folders = json.loads(f.read())
                prefix_set = set(get_prefix(id) for id in repo_folders)
                repo_folders = [id for id in repo_folders if get_prefix(id) in prefix_set]
            else:
                repo_folders = list_folders(args.repo_path)
                prefix_set = set(get_prefix(folder) for folder in repo_folders)
                repo_folders = [folder for folder in repo_folders if get_prefix(folder) in prefix_set]
        ...
        if args.download_repo:
            ...
            bench_data = load_dataset(args.dataset, split=args.split)
            if args.instance_id_path and osp.exists(args.instance_id_path):
                print("instance_id list loaded successful!")
                with open(args.instance_id_path, 'r') as f:
                    repo_folders = json.loads(f.read())
                for instance in bench_data:
    ...
    print("total graph number: ", len(repo_folders))
    manager = mp.Manager()
    queue = manager.Queue()
