...
def embed_text(tokenizer, model, text):
    ...
    with torch.no_grad():
        embeddings = model(**inputs).last_hidden_state.mean(dim=1)  # Saving embeddings pooled in the sequence dim
    return embeddings.cpu().numpy()  # to save embeddings

def embed_graph(tokenizer, model, graph):
    for node, attrs in graph.nodes(data=True):
        if attrs['type'] == "function":
            text_to_embed = attrs.get('code', node)  # embeds code attr
            embedding = embed_text(tokenizer, model, text_to_embed)
            attrs['embedding'] = embedding
    return graph

def compute_query_similarities(query_embedding, graph):
    """Compute cosine similarities between query and all node embeddings."""
    # Collect all node embeddings and their IDs
    node_ids = []
    embeddings = []
    for node, attrs in graph.nodes(data=True):
        if 'embedding' in attrs:
            node_ids.append(node)
            embeddings.append(attrs['embedding'].flatten())
    ...
    print(f"Query embedding ndim: {query_embedding.ndim}")
    print(f"Embeddings ndim: {embeddings.ndim}")
    similarities = cosine_similarity(query_embedding, embeddings)[0]
    
    # Create a dictionary of node IDs and their similarities
    similarity_dict = dict(zip(node_ids, similarities))
    
    return similarity_dict

def visualize_embeddings_2d(graph, query_embedding, output_file="plots/embedding_plot.png"):
    """Visualize query and node embeddings in 2D using t-SNE."""
    ...
    # Add query embedding
    embeddings.append(query_embedding.flatten())
    node_ids.append('Query')
    ...
    # Define colors and markers for node types
    type_colors = {
        'directory': 'orange',
        'file': 'lightgrey',
        'class': 'lightgreen',
    ...
    for ntype in set(node_types):
        ...
        plt.scatter(
            ...
            c=type_colors.get(ntype, 'grey'),
            marker=type_markers.get(ntype, 'o'),
            label=ntype,
            s=100 if ntype != 'query' else 200
        )
    
    # Add labels for query and top 5 similar nodes
    top_k = 5
    similarities = compute_query_similarities(query_embedding, graph)
    sorted_nodes = sorted(similarities.items(), key=lambda x: x[1], reverse=True)[:top_k]
    query_idx = len(node_ids) - 1  # Query is the last embedding
    
    for i, node_id in enumerate(node_ids):
        if node_id == 'Query' or node_id in [n[0] for n in sorted_nodes]:
            plt.annotate(
                node_id if node_id != 'Query' else 'Query',
    ...
    plt.legend()
    plt.title("2D Visualization of Embeddings with Query")
...
def analyze_code_length_distribution(tokenizer_path, graph):
    lengths = []
    if tokenizer_path:
        print("use tokenizer for word count")
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, trust_remote_code=True)
        for node, data in graph.nodes(data=True):
            if 'code' in data and data['type'] == "function":
                tokens = tokenizer.encode(data['code'])
                code_length = len(tokens)
                lengths.append(code_length)
    else:
        for node, data in graph.nodes(data=True):
            if 'code' in data:
    ...
    if not lengths:
        print("No nodes with 'code' attribute found.")
        return
    
    # 计算分布（使用 Counter）, 打印分布（排序输出）
    length_distribution = Counter(lengths)
    print("Code Length Distribution:")
    for length, count in sorted(length_distribution.items()):
        print(f"Length {length}: {count} nodes")

    # 可视化分布（使用 matplotlib 直方图）
    plt.figure(figsize=(10, 6))
    plt.hist(lengths, bins=8, edgecolor='black')  # bins 可以调整
    plt.title('Distribution of Code Lengths in Nodes')
    plt.xlabel('Code Length')
    plt.ylabel('Number of Nodes')
    plt.grid(True)
    plt.savefig('plots/code_length_distribution.png')  # 保存为图片
    plt.show()  # 或 plt.show() 显示


def main():
    # Generate Dependency Graph
    # try:
    #     with open(args.saved_path, 'rb') as f:
    #         graph = pickle.load(f)
    # except:
    ...
    if args.load_embedding:
        ...
        directory_count = 0
        for node, attrs in graph.nodes(data=True):
            if attrs['type'] == "function" and func_count < 5:
                text_to_embed = attrs.get('code', node)
                print(f"func {func_count}: {text_to_embed}")
                func_count += 1
            if attrs['type'] == "class" and class_count < 5:
                text_to_embed = attrs.get('code', node)
                print(f"class {class_count}: {text_to_embed}")
                class_count += 1
            if attrs['type'] == "file" and file_count < 5:
                text_to_embed = attrs.get('code', node)
                print(f"file {file_count}: {text_to_embed}")
                file_count += 1
            if attrs['type'] == "directory" and directory_count < 5:
                text_to_embed = attrs.get('code', node)
                print(f"directory {directory_count}: {text_to_embed}")
                directory_count += 1
    else:
        graph = build_graph(args.repo_path, global_import=args.global_import)
        print("graph built, start embedding...")

        start_time = time.time()
        graph = embed_graph(tokenizer, model, graph)
        end_time = time.time()
        execution_time = end_time - start_time
        print(f"time cost of graph: {execution_time:.3f} s")
        with open(args.saved_path, 'wb') as f:
            pickle.dump(graph, f)
    ...
    # query = "It handles the transformation and compilation of the RHS value for SQL query generation.The function first checks if bilateral transformations need to be applied to the value. If so, it wraps the raw value in a Value expression and applies the necessary transformations. It then resolves the resulting expression within the query context.For SQL compilation, the code checks if the value has SQL generation capability (as_sql method). If it does, it compiles the value to SQL and parameters, with special handling for certain expression types. It ensures proper parentheses wrapping for operator precedence while avoiding double-wrapping that could cause issues on some database backends like SQLite.If the value cannot be compiled to SQL directly, the function falls back to using Django's standard database preparation method for lookup values, which handles proper value escaping and formatting for the database connection.The code essentially bridges Django's ORM expressions with raw SQL generation while maintaining database compatibility and proper query structure."
    start_time = time.time()
    query = "Cause 4: Layered .values() Calls Confusing ORM Logic The chained .values('email').annotate(...).values('m') in a creates a complex queryset structure. The final .values('m') obscures the fact that the aggregation initially depended on email. When generating the subquery, the ORM may only consider the last .values() and thus group by m (or its source, id) instead of tracing back to the original email grouping."
    query_embedding = embed_text(tokenizer, model, query)
    similarities = compute_query_similarities(query_embedding, graph)
    end_time = time.time()
    execution_time = end_time - start_time
    print(f"time cost of retrieval: {execution_time:.3f} s")
    # Print top 5 similar nodes
    sorted_similarities = sorted(similarities.items(), key=lambda x: x[1], reverse=True)[:10]
    print("Top 10 similar nodes to query:")
    for node, sim in sorted_similarities:
        print(f"Node: {node}, Similarity: {sim:.4f}")
    visualize_embeddings_2d(graph, query_embedding)
    
    # analyze_code_length_distribution(args.model_path, graph)
